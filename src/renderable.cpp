...
renderable::renderable(std::vector<renderable *> &my_vector,
                       sf::Vector2f position, int width, int height,
                       std::string spritesheet_path, int render_width,
                       int render_height)
    : _my_vector(my_vector), x(position.x), y(position.y), _width(width),
      _height(height), _render_width(render_width),
...
sf::Vector2f renderable::get_position() { return sf::Vector2f(x, y); }
